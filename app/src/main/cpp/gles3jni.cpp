...
bool Renderer::loadShaderInText(const char *vertexShader, const char *fragmentShader) {
    return this->loadShaderInText(vertexShader, fragmentShader);
}
...
JNIEXPORT void JNICALL Java_com_android_gles3jni_GLES3JNILib_initWithShader(JNIEnv *env, jobject obj, jstring vertexShader, jstring fragmentShader);
...
JNIEXPORT void JNICALL Java_com_android_gles3jni_GLES3JNILib_setShaderInText(JNIEnv *env, jobject obj, jstring vertexShader, jstring fragmentShader);
};
...
JNIEXPORT void JNICALL
Java_com_android_gles3jni_GLES3JNILib_initWithShader(JNIEnv *env, jobject obj, jstring vertexShader, jstring fragmentShader) {
    if (g_renderer) {
        delete g_renderer;
        g_renderer = NULL;
    }
    ...
    const char *versionStr = (const char *) glGetString(GL_VERSION);
    if (strstr(versionStr, "OpenGL ES 3.") && gl3stubInit()) {
        g_renderer = createES3RendererWithShader(
                env->GetStringUTFChars(vertexShader, 0),
                env->GetStringUTFChars(fragmentShader, 0)
        );
    } else if (strstr(versionStr, "OpenGL ES 2.")) {
        g_renderer = createES2RendererWithShader(
                env->GetStringUTFChars(vertexShader, 0),
                env->GetStringUTFChars(fragmentShader, 0)
        );
    } else {
...
JNIEXPORT void JNICALL
Java_com_android_gles3jni_GLES3JNILib_setShaderInText(JNIEnv *env, jobject obj, jstring vertexShader, jstring fragmentShader) {
    if (g_renderer) {
        g_renderer->loadShaderInText(
                env->GetStringUTFChars(vertexShader, 0),
                env->GetStringUTFChars(fragmentShader, 0));
    }
}
