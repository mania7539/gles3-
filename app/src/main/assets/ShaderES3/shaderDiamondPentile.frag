...
// layouts/diamond_pentile/getSubpixelPos.frag, diamond pentile ------------------------------------------------------------
vec3 getSubpixelPos(float origin){
    // From pixel center find each subpixel center
    float g = (origin - 0.3 - 0.3*tan(LensDriftAngleRad));
    float r, b;
    if (int(panelPos.x+panelPos.y+0.1)%2 == 1){
        // GB site
        r = origin + 0.225 + 0.225*tan(LensDriftAngleRad) - 1.0;
        b = origin + 0.225 + 0.225*tan(LensDriftAngleRad);
    } else {
        // GR site
        r = origin + 0.225 + 0.225*tan(LensDriftAngleRad);
        b = origin + 0.225 + 0.225*tan(LensDriftAngleRad) - 1.0;
    }
